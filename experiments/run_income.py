...
from ppci_condmean.data import load_census_npz, census_sex_subset, nw_oracle_mean
...
def stable_target_offset(sex: int, age: int) -> int:
    return int(100000 * int(sex) + 1000 * int(age))


def make_cfg(
    args,
    *,
    pc_r: float | None = None,
    mb_gamma: float | None = None,
    gh_gamma: float | None = None,
) -> JointTuningConfig:
...
def aggregate_from_targets(target_summary: pd.DataFrame) -> pd.DataFrame:
    rows = []
    ratio_cols = [c for c in target_summary.columns if c.startswith("width_ratio_vs_") or c.startswith("rmse_ratio_vs_")]
    for method, g in target_summary.groupby("method", dropna=False):
        row = {
            "method": method,
            "n_targets": int(len(g)),
            "coverage_mean": float(g["coverage"].mean()),
            "coverage_min_target": float(g["coverage"].min()),
            "coverage_max_target": float(g["coverage"].max()),
            ...
            "k_target_mean": float(g["k_target_mean"].mean()) if g["k_target_mean"].notna().any() else np.nan,
            "R_GH_mean": float(g["R_GH_mean"].mean()) if g["R_GH_mean"].notna().any() else np.nan,
            "R_MB_mean": float(g["R_MB_mean"].mean()) if g["R_MB_mean"].notna().any() else np.nan,
...
def compact_tuning_decisions(df: pd.DataFrame, k_max_frac: float) -> pd.DataFrame:
    ...
    if tuned.empty:
        return pd.DataFrame()
    keys = ["sex", "age", "method", "unlab_rep"]
    cols = keys + [
        "n_label", "n_unlab", "setting", "tuning_status", "lambda_factor", "h_factor",
        "k_target_mean", "R_MB_mean", "R_GH_mean", "bias_score_mean", "bias_budget_mean",
        "gh_gamma_eff_mean", "edge_score_mean",
        "pc_ratio_mean", "J_w_mean", "sw_proxy_mean", "op_score", "loc_score",
        "tune1_k_target", "tune2_k_target", "tune1_status", "tune2_status",
    ]
    ...
    if "tune1_k_target" in out and "tune2_k_target" in out:
        fold_m = (out["n_unlab"].astype(int) // 2).clip(lower=1)
        out["k_max"] = np.floor(float(k_max_frac) * fold_m).clip(2).astype(int)
        out["boundary"] = (
            (out["tune1_k_target"].astype(float) >= out["k_max"].astype(float))
            | (out["tune2_k_target"].astype(float) >= out["k_max"].astype(float))
        )
    else:
        out["boundary"] = np.nan
    return out
...
def summarize_tuning_decisions(tune: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    ...
    rows = []
    value_cols = [
        "lambda_factor", "h_factor", "k_target_mean", "R_MB_mean", "R_GH_mean",
        "bias_score_mean", "bias_budget_mean", "gh_gamma_eff_mean", "edge_score_mean", "pc_ratio_mean", "J_w_mean",
        "sw_proxy_mean", "op_score", "loc_score",
    ]
    for key, g in tune.groupby(group_cols + ["method"], dropna=False):
        ...
        row.update({
            ...
            "boundary_rate": float(g["boundary"].mean()) if "boundary" in g else np.nan,
        })
...
def run_one_target(sex: int, age: int, cached_data: tuple, args) -> list[dict]:
    X_raw, X_std, Y_s, f_s, mean, std, all_idx = cached_data
    ...
    theta0 = nw_oracle_mean(X_std, Y_s, x0, kernel=args.kernel)
    mask_x0 = np.all(X_raw == x0_raw.reshape(1, -1), axis=1)
    pool = all_idx[~mask_x0]
    need = int(args.n_label + args.n_unlab)
    if len(pool) < need:
        raise ValueError(f"Not enough census observations for sex={sex}, age={age}: pool={len(pool)}, need={need}")

    rows = []
    for urep in range(args.unlab_reps):
        seed_u = int(args.seed + stable_target_offset(sex, age) + 10000 * urep)
        ...
        if len(label_pool) < args.n_label:
            raise ValueError(f"Not enough census observations after unlabeled draw for sex={sex}, age={age}: label_pool={len(label_pool)}, n_label={args.n_label}")
        X_u = X_std[un_idx]
        f_u = f_s[un_idx]
        ...
        for brep in range(args.label_reps):
            ...
            seed_l = int(args.seed + stable_target_offset(sex, age) + 5000000 + 10000 * urep + brep)
            ...
            f_l = f_s[lab_idx]
            ctx = {
                "experiment": "census_joint",
                "setting": f"sex{sex}_age{age}",
                "sex": int(sex),
                "age": int(age),
                "rep": int(rep),
                "unlab_rep": int(urep),
    ...
    print(f"[done] sex={sex}, age={age}, theta0={theta0:.4f}, unlab_reps={args.unlab_reps}, label_reps={args.label_reps}", flush=True)
    return rows
...
def write_report(out_dir: Path, args, aggregate: pd.DataFrame, by_sex: pd.DataFrame, tuning_method: pd.DataFrame):
    report = out_dir / "report.md"
    gh_label = "GH"
    gh_label = first_gh_label(args)
    lines = [
        "# Census Income Experiment Summary",
        "",
        "## Setup",
        "",
        f"- Data: `{args.data}`",
        f"- Targets: ages `{args.ages}`, sexes `{args.sexes}`",
        f"- Sampling: `n_label={args.n_label}`, `n_unlab={args.n_unlab}`, `unlab_reps={args.unlab_reps}`, `label_reps={args.label_reps}` (`reps={args.reps}` total) per age-sex target",
        ...
        "- Bias/coverage are computed relative to the same NW-oracle target used by the earlier census script.",
        "",
        "## Equal-Target Aggregate",
        "",
        markdown_table(
            aggregate,
            [
                "method", "n_targets", "coverage_mean", "coverage_min_target", "bias_mean",
        ...
        markdown_table(
            by_sex,
            [
                "sex", "method", "n_reps", "n_unlab_clusters", "coverage",
                "coverage_cluster_se", "bias", "abs_bias", "rmse", "width", "fallback_rate",
            ],
        ),
        "## Tuning Diagnostics",
        "",
        markdown_table(
            tuning_method,
            [
                "method", "tuning_decisions", "fallback_rate", "boundary_rate",
                "lambda_factor_mean", "h_factor_mean", "k_target_mean",
                "R_GH_mean", "R_MB_mean", "bias_score_mean", "bias_budget_mean", "gh_gamma_eff_mean", "pc_ratio_mean",
            ],
        ),
    ]
...
def main():
    ap = argparse.ArgumentParser(description="Census income experiment with P1 covariate-only tuning.")
    ap.add_argument("--data", default="data/census_income/census_income.npz")
    ap.add_argument("--output-dir", default="runs/income_sex1")
    ap.add_argument("--seed", type=int, default=15100)
    ...
    ap.add_argument("--ages", default=",".join(str(x) for x in range(70, 101)))
    ap.add_argument("--sexes", default="1")
    ...
    ap.add_argument("--h-factors", default="1.0,1.2,1.4")
    ap.add_argument("--bias-screens", default="p1_label", help="Comma-separated: p1_label,p2_log,p3_full,legacy")
    ap.add_argument("--c-biases", default="22", help="Comma-separated c_bias values for A_bias.")
    ...
    ages = parse_ints(args.ages)
    sexes = parse_ints(args.sexes)
    cache = {}
    tasks = []
    for sex in sexes:
        X_raw, X_std, Y_s, f_s, mean, std = census_sex_subset(X, Y, f, sex)
        all_idx = np.arange(len(Y_s))
        cache[sex] = (X_raw, X_std, Y_s, f_s, mean, std, all_idx)
        for age in ages:
            tasks.append((sex, age))
    ...
    if args.workers and args.workers > 1 and len(tasks) > 1:
        try:
            ...
            nested = Parallel(n_jobs=args.workers, backend="loky")(
                delayed(run_one_target)(sex, age, cache[sex], args) for sex, age in tasks
            )
            ...
            rows = []
            for sex, age in tasks:
                rows.extend(run_one_target(sex, age, cache[sex], args))
    else:
        rows = []
        for sex, age in tasks:
            rows.extend(run_one_target(sex, age, cache[sex], args))
    ...
    gh_label = first_gh_label(args)
    by_target = summarize_replicates(rep, ["sex", "age"])
    by_target = add_ratios(by_target, ["sex", "age"], gh_label)
    by_target = add_ratios(by_target, ["sex", "age"], "INC")
    by_target_path = out_dir / "summary_by_target.csv"
    by_target.to_csv(by_target_path, index=False)

    by_sex = summarize_replicates(rep, ["sex"])
    by_sex = add_ratios(by_sex, ["sex"], gh_label)
    by_sex_path = out_dir / "summary_by_sex.csv"
    by_sex.to_csv(by_sex_path, index=False)

    aggregate = aggregate_from_targets(by_target)
    aggregate_path = out_dir / "aggregate.csv"
    aggregate.to_csv(aggregate_path, index=False)

    tune = compact_tuning_decisions(rep, args.k_max_frac)
    tune_path = out_dir / "tuning_decisions.csv"
    tune.to_csv(tune_path, index=False)
    tune_by_target = summarize_tuning_decisions(tune, ["sex", "age"])
    tune_by_target_path = out_dir / "tuning_by_target.csv"
    tune_by_target.to_csv(tune_by_target_path, index=False)
    ...
    write_report(out_dir, args, aggregate, by_sex, tune_by_method)
    print(f"Saved {by_target_path}")
    print(f"Saved {by_sex_path}")
    print(f"Saved {aggregate_path}")
    print(f"Saved {tune_path}")
    print(f"Saved {tune_by_target_path}")
